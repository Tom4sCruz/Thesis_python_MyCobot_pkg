...
collaborative robot:
  ...
  * a scripted NUDGE / flinch (as the arm nears a cube it RECOILS, waits for the
    ...
    DISABLED here (NUDGE_CYCLE = -1). Set NUDGE_CYCLE to an even reach cycle to
    enable it. Fully scripted -- the arm has no sensors.
...
# DISABLED for this version (-1). To enable: set NUDGE_CYCLE to an even reach
# cycle -- 4 = 3rd cube, 6 = 4th cube (N_CYCLES = 2 * n_cubes). run_nudge then
# does approach-to-fraction -> recoil hop -> settle -> re-approach the moved cube.
